/**
  ...
  * @author  rma 10.10.2015 REV D
  * @version V3.1.2
  * @date    10.10.2015 V3.1.2
	* @date    
  * @brief   Main. Tested ADC und Timer. Sendet die Daten via RS232 1 oder USART 1
	*          RS232    : ACHTUNG Pegel sind -15V (1) ... +15V (0) an den 4 poligen UART Steckern.
	*          RS232-TTL: Standard Pegel (3.3V) können an PB6 (Transmitt) und PB7 (Receive)  abgenommen werden.
	*                     ACHTUNG:    die Pegel an den 4poligen Steckern benötigen ein echtes RS232 Interface als Gegenüber.
	*                                 Sonst kann die Gegenstelle oder sogar der Computer SCHADEN nehmen !!!!!!!!!!!
	*                     Empfehlung: Die Signale zuerst an PB6 / PB7 mit KO beobachten und eventuell via einen
	*                                 FTDI TTL KIT an einen USB Port des PC anschliessen.
...
unsigned int start_time, stop_time, cycle_count, ui_del1, ui_del2;  // used for Execution Time purposes
...
/**
  * @brief  Private Function for RMA for HEX to ASCII Conversion.
...
/* Configuration Error and Capture error */                  // rma_06072014 Wenn der Zähler fehler meldet können die hier abgefangen werden. Configuration Error
  ...
  ET_STM32_LED_On(LED7);  // rma 
	ET_STM32_LED_On(LED5);  // rma 
  ET_STM32_LED_On(LED1);  // rma   Reset Leitung Bluetooth
...
// Clear Screen GLCD 
  ...
  LCD_DisplayStringLine(Line8, "-->  Zeit und ADC   ");
  LCD_DisplayStringLine(Line9, "rma/1504aa  ,  REV D");
	delay_ms(1500);
  ...
  while(1)													// Loop Continue
  {
	...
	if (uart_rx>0)
	{
      ...
      case '3' : LCD_Clear(White);
				   ...
				   // Test RTC or RS232 and Dosplay. extended Test
...
void test_adc_mcb32(void)
{
  ...
  {    
    /* obtain adc converted value of potmeter position */
    ADCVal = ADC_GetConversionValue(ADC1);
	  // rma off because of bluetooth resetET_STM32_LED_Write(led_graph[ADCVal/512]); 			//Display ADC to Bargraph LED 
  }
}
...
//  sprintf(uart_buf,"\n\n\n RTC Testing...\n");	
//	printUART1((unsigned int *)uart_buf);
//  // rma printUART2((unsigned int *)uart_buf);
...
/************************************************/
/* 4 : Test RTC MCB32                           */
/* Test RTC or RS232 and Display. extended Test */
/************************************************/
// rma_21122013 with ADC, TImer and RS232 Testing
...
void test_rtc_mcb32(void)
{
  ...
  /* Enable the RTC Second */
	// see RTC_IRQHandler where the Interrupt is handled. Currently, the IRSRoutine toogles the LED0
  ...
  /* Clear reset flags */
  RCC_ClearFlag();
	LCD_DisplayStringLine(Line2, "  WAIT: Reset Module");
	ET_STM32_LED_On(LED0);  // rma   Reset OFF Leitung Bluetooth
  delay_ms(500);
	ET_STM32_LED_Off(LED0); // rma   Reset ON  Leitung Bluetooth
	delay_ms(500);
	ET_STM32_LED_On(LED0);  // rma   Reset Off Leitung Bluetooth
	delay_ms(4000);
	LCD_DisplayStringLine(Line2, "  Module reset OK   ");
	delay_ms(500);
  LCD_DisplayStringLine(Line2, "       Zeit/ADC Demo");
  // rma while ((TSS%12)<11)    // rma_21122013 
	sprintf(uart_buf,"Zeit/ADC Demo...\n");	
  printUART1((unsigned int *)uart_buf);
	while(1)
  {
	   /* If 1s has passed */
    if (TimeDisplay == 1)            // rma TimeDisplay wird in  RTC_IRQHandler gesetzt wenn eine Sekunde vorbei ist.
    {
      ...
      TimeDisplay = 0;
		...
		// rma_21122013  ADC Test hier eingefügt. Das heisst der ADC läuft so schnell wie möglich.
		SysTick -> CTRL=0;
	  ...
	  start_time =SysTick ->VAL;   // Stoppuhr gestartet  rma_05072014
		...
		// RS232 Testing. Set the baudrate in "et_stm32f_arm_kit_uart.c" aprox. line 38
		sprintf(uart_buf,"\nTime: %0.2d:%0.2d:%0.2d\n", THH, TMM, TSS);	                  //  sprintf(uart_buf,"5A");  // Code for testing 
    printUART1((unsigned int *)uart_buf);     // send the data via PB6 (TXD2 from USART1) and TX of RS232-1
		for(ui_del1=0;ui_del1<100;ui_del1++)
			{
		   for(ui_del2=0;ui_del2<100000;ui_del2++);
		  };
		// send CR to UART1, CR = carriage Return code (ASCII 13dez)
		sprintf(uart_buf,"%c", 13);	                  //  sprintf(uart_buf,"5A");  // Code for testing 
    printUART1((unsigned int *)uart_buf);     // send the data via PB6 (TXD2 from USART1) and TX of RS232-1
			// mit delay geht es nicht. Vermutlich IR Probleme  delay_ms(100);
			
		// printUART2((unsigned int *)uart_buf);     // send the data via PD5 (TXD2 from USART2) and TX of RS232-2
