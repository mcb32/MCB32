 ...
 *  Read the original Notes from Bosch on Github
 *  Adapted for MCB32  4.4.2019 Malacarne / Cityline AG
 * File     bme280_CZ.c
 * Date     08 Mar 2019
 * Version  3.3.6
 *
...
{
    int8_t rslt;
	  uint16_t temp_len;       // moved position rma 13.3.2019 CZ
    uint8_t reg_addr_cnt;    // moved position rma 13.3.2019 CZ
                               ...
                               const struct bme280_dev *dev)
{
    ...
    {
        rslt = set_osr_humidity_settings(settings, dev);                      // hier passiert es, auf die Adresse wird eine 01 geschrieben
    }
    if (desired_settings & (BME280_OSR_PRESS_SEL | BME280_OSR_TEMP_SEL))
    {
        rslt = set_osr_press_temp_settings(desired_settings, settings, dev);    // hier wird eine 54 Hex auf die Adresse geschrieben
    }
